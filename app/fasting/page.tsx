...
const S = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .navbar{position:sticky;top:0;z-index:100;background:rgba(12,10,6,.95);backdrop-filter:blur(16px);border-bottom:1px solid var(--border);padding:0 24px;display:flex;align-items:center;justify-content:space-between;height:56px}
    .nav-logo{display:flex;align-items:center;gap:8px;text-decoration:none;font-family:'Syne',sans-serif;font-size:17px;font-weight:800;background:linear-gradient(135deg,#a78bfa,#38bdf8);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
    ...
    .nav-link:hover{background:rgba(245,158,11,.1);color:var(--amber)}
    .hero{background:linear-gradient(160deg,#0c0a06 0%,#1c1003 50%,#0f0800 100%);padding:44px 20px 36px;text-align:center;position:relative;overflow:hidden;border-bottom:1px solid var(--border)}
    .hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 50% 0%,rgba(245,158,11,.12) 0%,transparent 70%)}
    .hero-icon{font-size:52px;display:block;margin-bottom:14px;position:relative;filter:drop-shadow(0 0 24px rgba(245,158,11,.6));animation:pulse 3s ease-in-out infinite}
    @keyframes pulse{0%,100%{filter:drop-shadow(0 0 24px rgba(245,158,11,.6))}50%{filter:drop-shadow(0 0 40px rgba(245,158,11,.9))}}
    .hero h1{font-family:'Syne',sans-serif;font-size:clamp(22px,4vw,38px);font-weight:800;background:linear-gradient(135deg,var(--amber-lt),var(--orange));-webkit-background-clip:text;-webkit-text-fill-color:transparent;position:relative;line-height:1.2}
    ...
    .badge{background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.25);color:var(--amber);font-size:16px;padding:3px 11px;border-radius:20px;font-weight:600}
    ...
    .proto-btn{background:var(--card2);border:1.5px solid var(--border);border-radius:12px;padding:14px 12px;cursor:pointer;transition:all .2s;text-align:center;font-family:'Space Grotesk',sans-serif}
    .proto-btn:hover{border-color:var(--amber-dim)}
    .proto-btn.sel{background:rgba(245,158,11,.1);border-color:var(--amber);box-shadow:0 0 0 3px rgba(245,158,11,.1)}
    ...
    .proto-pop{font-size:15px;font-weight:700;color:var(--amber);background:rgba(245,158,11,.15);padding:2px 7px;border-radius:10px;display:inline-block;margin-top:4px}
    ...
    input[type=time]{width:100%;padding:13px 16px;background:var(--card2);border:1.5px solid var(--border);border-radius:10px;color:var(--text);font-family:'Space Grotesk',sans-serif;font-size:22px;font-weight:700;transition:border-color .2s;-webkit-text-fill-color:var(--text)}
    ...
    .pill{padding:7px 14px;border-radius:22px;border:1.5px solid var(--border);font-size:15px;font-weight:500;cursor:pointer;transition:all .2s;background:transparent;color:var(--muted);font-family:'Space Grotesk',sans-serif}
    ...
    .btn-calc{background:linear-gradient(135deg,var(--amber-dim),var(--amber));color:#0c0a06;box-shadow:0 4px 20px rgba(245,158,11,.35)}
    ...
    .tl-eat{background:linear-gradient(90deg,rgba(22,163,74,.6),rgba(22,163,74,.3));display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:#4ade80;transition:width .5s}
    .tl-fast{background:linear-gradient(90deg,rgba(245,158,11,.3),rgba(245,158,11,.6));display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:var(--amber);transition:width .5s}
    ...
    .warn-banner{background:#1a0a0a;border:2px solid #ef4444;border-radius:12px;padding:16px;margin-bottom:16px}
    ...
    .diabetes-box{background:#0a1628;border:1.5px solid #3b82f6;border-radius:10px;padding:14px;margin-top:10px}
    ...
    .quiz-opt{background:var(--card2);border:1.5px solid var(--border);border-radius:9px;padding:10px 13px;cursor:pointer;transition:all .2s;font-size:15px;color:var(--muted);font-family:'Space Grotesk',sans-serif;text-align:left}
    ...
    .quiz-result{background:rgba(245,158,11,.1);border:1.5px solid var(--amber);border-radius:10px;padding:14px;margin-bottom:12px}
    ...
    .skip-link{font-size:14px;color:var(--muted);text-decoration:underline;cursor:pointer;background:none;border:none;font-family:'Space Grotesk',sans-serif;margin-top:6px;display:block}
...
export default function IFCalc() {
  ...
  const getRecommendation = () => {
    if (quizQ1 === "never") return PROTOCOLS[3];
    if (quizQ1 === "tried" && quizQ2 === "lose") return PROTOCOLS[0];
    if (quizQ1 === "tried" && quizQ2 === "health") return PROTOCOLS[0];
    if (quizQ1 === "tried" && quizQ2 === "max") return PROTOCOLS[1];
    if (quizQ1 === "doing" && quizQ2 === "lose") return PROTOCOLS[1];
    if (quizQ1 === "doing" && quizQ2 === "max") return PROTOCOLS[2];
    return PROTOCOLS[0];
  };

  const calculate = () => {
    if (proto.special === "52") { setResults({ special: "52" }); return; }
    ...
    <div className="app">
      ...
      <div className="main">
        <div className="warn-banner">
          ...
          <div className="warn-banner-text">Intermittent fasting is <strong style={{color:"#fca5a5"}}>not suitable for everyone</strong>. Do not start any fasting protocol if you:</div>
          ...
          <div className="diabetes-box">
            ...
            <div className="diabetes-box-text">If you have diabetes, fasting can cause dangerous drops in blood sugar (hypoglycemia), especially if you take insulin or certain oral medications. The timing of your meals is directly tied to your medication schedule. <strong style={{color:"#93c5fd"}}>Always consult your doctor before changing your eating schedule.</strong> If your doctor approves fasting, the 14:10 Gentle Fast is generally the safest starting point.</div>
          </div>
        </div>
        ...
        {showQuiz && !quizDone && (
          <div className="quiz-card">
            ...
            <div className="quiz-opts">
              {[{v:"never",l:"No — I'm completely new to fasting"},{v:"tried",l:"I've tried it a few times"},{v:"doing",l:"Yes — I already fast regularly"}].map(o => (
            ...
            {quizQ1 && (<>
              ...
              <div className="quiz-opts">
                {[{v:"lose",l:"⚖️ Lose weight"},{v:"health",l:"🌿 Improve general health"},{v:"max",l:"🔬 Maximum results / autophagy"}].map(o => (
            ...
            {quizQ1 && quizQ2 && (<>
              ...
              <button className="btn btn-calc" onClick={() => { setProto(getRecommendation()); setQuizDone(true); setShowQuiz(false); }}>☀️ Use This Protocol</button>
            </>)}
            <button className="skip-link" onClick={() => setShowQuiz(false)}>Skip — I know which protocol I want</button>
          </div>
        )}
