...
const S = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    /* NAV BAR */
    .navbar{position:sticky;top:0;z-index:100;background:rgba(6,13,15,.95);backdrop-filter:blur(16px);border-bottom:1px solid var(--border);padding:0 24px;display:flex;align-items:center;justify-content:space-between;height:56px}
    ...
    .nav-link:hover{background:rgba(20,184,166,.1);color:var(--teal)}

    /* HERO */
    .hero{background:linear-gradient(160deg,#060d0f 0%,#091518 60%,#060d0f 100%);padding:44px 20px 36px;text-align:center;position:relative;overflow:hidden;border-bottom:1px solid var(--border)}
    .hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 50% 0%,rgba(20,184,166,.1) 0%,transparent 65%)}
    .hero-icon{font-size:52px;display:block;margin-bottom:14px;position:relative;filter:drop-shadow(0 0 24px rgba(20,184,166,.5))}
    .hero h1{font-family:'Syne',sans-serif;font-size:clamp(22px,4vw,36px);font-weight:800;color:var(--text);position:relative;line-height:1.2}
    .hero h1 span{background:linear-gradient(135deg,var(--teal-lt),var(--blue));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
    ...
    .badge{background:rgba(20,184,166,.1);border:1px solid rgba(20,184,166,.2);color:var(--teal);font-size:16px;padding:3px 11px;border-radius:20px;font-weight:600}
    ...
    /* QUESTION */
    .q-number{font-size:16px;font-weight:700;color:var(--teal);text-transform:uppercase;letter-spacing:.7px;margin-bottom:8px}
    ...
    .option{display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--card2);border:1.5px solid var(--border);border-radius:10px;cursor:pointer;transition:all .2s;font-family:'Space Grotesk',sans-serif}
    .option:hover{border-color:var(--teal-dim)}
    .option.sel{background:rgba(20,184,166,.1);border-color:var(--teal);box-shadow:0 0 0 2px rgba(20,184,166,.1)}
    .option-dot{width:18px;height:18px;border-radius:50%;border:2px solid var(--border);flex-shrink:0;transition:all .2s;display:flex;align-items:center;justify-content:center}
    ...
    .btn{flex:1;padding:12px;border-radius:10px;border:none;font-family:'Space Grotesk',sans-serif;font-size:16px;font-weight:700;cursor:pointer;transition:all .2s;letter-spacing:.3px}
    .btn-back{background:var(--card2);color:var(--soft);border:1px solid var(--border);flex:0.4}
    .btn-next{background:linear-gradient(135deg,var(--teal-dim),var(--teal));color:#fff;box-shadow:0 4px 16px rgba(20,184,166,.3)}
    .btn-next:hover{transform:translateY(-1px)}
    .btn-next:disabled{opacity:.3;cursor:not-allowed;transform:none}
    .btn-submit{background:linear-gradient(135deg,var(--teal-dim),var(--teal));color:#fff;box-shadow:0 4px 20px rgba(20,184,166,.35);width:100%;padding:14px;font-size:14px;margin-top:6px}
    .btn-reset{background:var(--card2);color:var(--soft);border:1px solid var(--border);width:100%;margin-top:12px}

    /* RESULTS */
    .score-ring{display:flex;flex-direction:column;align-items:center;padding:28px 20px;border-radius:14px;margin-bottom:14px;position:relative;overflow:hidden}
    ...
    .rec-item{display:flex;gap:12px;padding:13px;background:var(--card2);border:1px solid var(--border);border-radius:10px;align-items:flex-start}
...
export default function BurnoutCalc() {
  ...
  const reset = () => { setCurrentQ(0); setAnswers({}); setResults(null); setSelected(null); };

  return (
    <div className="app">
      <S />

      {/* NAV BAR — Stress & Burnout excluded (current page) */}
      ...
      <div className="seo-wrap">
        ...
        <div className="seo-card">
          ...
          <div className="seo-step"><div className="seo-num">2</div><div className="seo-txt"><strong>Rate each statement</strong> — choose from four response options ranging from Never to Almost Always. Each response is weighted to give a nuanced picture of your situation.</div></div>
