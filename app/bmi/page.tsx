  ...
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    @media(max-width:520px){
      ...
      .pi-ranges{grid-template-columns:1fr 1fr}
    }
  `}</style>
);
...
export default function App() {
  ...
  const getThresholds = (eth) => eth === "asian"
    ? { uw: 18.5, normal: 23, ow: 27.5 }
    : { uw: 18.5, normal: 25, ow: 30 };

  const getBMIColor = (bmi, eth) => {
    const t = getThresholds(eth);
    if (bmi < t.uw) return "#38bdf8";
    if (bmi < t.normal) return "#10b981";
    if (bmi < t.ow) return "#f59e0b";
    if (bmi < 35) return "#f97316";
    return "#ef4444";
  };

  const getBMICategory = (bmi, eth) => {
    const t = getThresholds(eth);
    if (bmi < t.uw) return { label: "Underweight", cls: null };
    if (bmi < t.normal) return { label: "Healthy Weight", cls: null };
    if (bmi < t.ow) return { label: "Overweight", cls: null };
    if (bmi < 35) return { label: "Obese", cls: "Class 1 — Moderate" };
    if (bmi < 40) return { label: "Obese", cls: "Class 2 — Severe" };
    return { label: "Obese", cls: "Class 3 — Very Severe" };
  };

  const getScalePos = (bmi) => Math.min(Math.max(((bmi - 15) / 30) * 100, 2), 98);

  const getHealthyRange = (hM) => ({
    lbs: `${(18.5 * hM * hM * 2.205).toFixed(0)} – ${(24.9 * hM * hM * 2.205).toFixed(0)} lbs`,
    kg: `${(18.5 * hM * hM).toFixed(1)} – ${(24.9 * hM * hM).toFixed(1)} kg`
  });

  const getRisks = (bmi, eth) => {
    const t = getThresholds(eth);
    if (bmi < t.uw) return [
      { t: "Malnutrition and vitamin or mineral deficiencies", c: "#38bdf8" },
      { t: "Weakened immune system and increased infection risk", c: "#38bdf8" },
      { t: "Osteoporosis and reduced bone density over time", c: "#38bdf8" },
      { t: "Hormonal imbalances that may affect fertility", c: "#38bdf8" },
    ];
    if (bmi < t.normal) return [
      { t: "You are in the healthy weight range — keep up your balanced habits", c: "#10b981" },
      { t: "Regular health check-ups are recommended at any weight", c: "#10b981" },
      { t: "BMI is just one indicator — waist size and fitness level also matter", c: "#10b981" },
    ];
    if (bmi < t.ow) return [
      { t: "Increased risk of Type 2 diabetes", c: "#f59e0b" },
      { t: "Higher blood pressure and cardiovascular risk", c: "#f59e0b" },
      { t: "Greater likelihood of sleep apnea", c: "#f59e0b" },
      { t: "Increased joint stress especially in knees and hips", c: "#f59e0b" },
    ];
    return [
      { t: "Significantly elevated risk of Type 2 diabetes", c: "#ef4444" },
      { t: "High blood pressure and heart disease", c: "#ef4444" },
      { t: "Increased risk of certain cancers", c: "#ef4444" },
      { t: "Sleep apnea and breathing problems", c: "#ef4444" },
      { t: "Joint disease and reduced mobility", c: "#ef4444" },
    ];
  };

  const getSteps = (bmi, eth) => {
    const t = getThresholds(eth);
    if (bmi < t.uw) return [
      { h: "Speak with your doctor", d: "Unexplained low weight can indicate underlying conditions. A doctor can identify causes and create a safe weight gain plan." },
      { h: "Increase calorie-dense whole foods", d: "Focus on nuts, avocados, whole grains, lean proteins and healthy oils — not just high-calorie processed foods." },
      { h: "Check your nutrient intake", d: "Use our Optimal Nutrients Calculator to identify deficiencies common in underweight individuals." },
      { h: "Consider resistance training", d: "Building muscle mass is a healthy and sustainable way to increase body weight over time." },
    ];
    if (bmi < t.normal) return [
      { h: "Maintain your healthy habits", d: "Whatever you're doing is working. Focus on sustainable nutrition and regular physical activity." },
      { h: "Track your nutrient intake", d: "A healthy weight doesn't guarantee optimal nutrition. Use our Optimal Nutrients Calculator." },
      { h: "Monitor waist circumference", d: "Even at a healthy BMI, carrying excess abdominal fat increases health risk significantly." },
      { h: "Prioritize sleep quality", d: "Poor sleep disrupts metabolism and weight regulation over time. Use our Sleep Cycle Calculator." },
    ];
    if (bmi < t.ow) return [
      { h: "Focus on small sustainable changes", d: "A 5–10% reduction in body weight can significantly reduce health risks without drastic measures." },
      { h: "Reduce ultra-processed foods", d: "Increase vegetables, lean proteins and whole grains. Cut back on processed and fast food." },
      { h: "Add regular movement", d: "Aim for 150 minutes of moderate activity per week. Walking absolutely counts." },
      { h: "Consider intermittent fasting", d: "Many people find structured eating windows helpful. Use our Intermittent Fasting Calculator to find the right protocol." },
    ];
    return [
      { h: "Consult your doctor", d: "At this BMI level medical guidance is important. Your doctor can rule out underlying conditions and discuss safe options." },
      { h: "Set realistic small goals", d: "Losing just 5–10% of body weight produces measurable improvements in blood pressure, blood sugar and energy." },
      { h: "Address sleep and stress", d: "Poor sleep and chronic stress drive weight gain through hormones. Use our Sleep Cycle and Stress & Burnout tools." },
      { h: "Consider professional support", d: "A registered dietitian can create a safe personalized plan. Medical interventions may also be appropriate to discuss." },
    ];
  };

  const getWaistRisk = (waistCm, s) => {
    if (!waistCm || !s) return null;
    if (s === "male") {
      if (waistCm < 94) return { status: "Low Risk", color: "#10b981" };
      if (waistCm < 102) return { status: "Increased Risk", color: "#f59e0b" };
      return { status: "High Risk", color: "#ef4444" };
    }
    if (waistCm < 80) return { status: "Low Risk", color: "#10b981" };
    if (waistCm < 88) return { status: "Increased Risk", color: "#f59e0b" };
    return { status: "High Risk", color: "#ef4444" };
  };

  const getPICategory = (pi) => {
    if (pi < 11) return { label: "Underweight", color: "#38bdf8" };
    if (pi < 14) return { label: "Healthy", color: "#10b981" };
    if (pi < 17) return { label: "Overweight", color: "#f59e0b" };
  ...
  const calculate = () => {
    ...
    setResults({
      bmi, pi, hM, wKg,
      category: getBMICategory(bmi, ethnicity),
      color: getBMIColor(bmi, ethnicity),
      hwRange: getHealthyRange(hM),
      risks: getRisks(bmi, ethnicity),
      steps: getSteps(bmi, ethnicity),
      waistRisk: getWaistRisk(waistCm, sex),
      piCat: getPICategory(pi),
      diffLbs: diff * 2.205,
    });
  };
  ...
  return (
    <div className="app">
      ...
      <div className={`mobile-menu${menuOpen ? " open" : ""}`}>
        ...
        <a href="/nutrients" className="mobile-link" onClick={() => setMenuOpen(false)}>💊 Optimal Nutrients</a>
              ...
              {(ethnicity === "asian" || ethnicity === "black") && (
                <div className="fg full">
                  <div className="eth-note">
                    {ethnicity === "asian"
                      ? "ℹ️ People of Asian descent develop obesity-related health risks at lower BMI levels. This calculator uses adjusted thresholds supported by international health research: Overweight at 23+, Obese at 27.5+."
                      : "ℹ️ Research suggests standard BMI thresholds may overestimate obesity-related risk for people of Black African or African-Caribbean descent. Results are shown with standard thresholds — discuss with your doctor for personalized guidance."}
                  </div>
                </div>
              )}
          ...
          <>
            ...
            <div className="card">
              ...
              <div className="stats-grid">
                <div className="stat">
                  <div className="stat-lbl">Your BMI</div>
                  <div className="stat-val" style={{ color: results.color }}>{results.bmi.toFixed(1)}</div>
                  <div className="stat-sub">kg/m²</div>
                </div>
                <div className="stat">
                  <div className="stat-lbl">Healthy Weight Range</div>
                  <div className="stat-val" style={{ fontSize: "14px", marginTop: "4px" }}>{results.hwRange.lbs}</div>
                  <div className="stat-sub">{results.hwRange.kg}</div>
                </div>
                <div className="stat">
                  <div className="stat-lbl">To Reach Healthy BMI</div>
                  <div className="stat-val" style={{ fontSize: "14px", marginTop: "4px" }}>
                    {Math.abs(results.diffLbs) < 4 ? "✓ At target weight" : results.diffLbs > 0 ? `Lose ${results.diffLbs.toFixed(0)} lbs` : `Gain ${Math.abs(results.diffLbs).toFixed(0)} lbs`}
                  </div>
                  <div className="stat-sub">to reach mid-healthy BMI</div>
                </div>
                <div className="stat">
                  <div className="stat-lbl">Category</div>
                  <div className="stat-val" style={{ fontSize: "14px", marginTop: "4px", color: results.color }}>{results.category.label}</div>
                  <div className="stat-sub">{results.category.cls || "Standard classification"}</div>
                </div>
              </div>
              ...
              {results.bmi >= 30 && (
                <div className="ob-box">
                  ...
                  <div className="ob-desc">
                    Obesity is divided into three classes based on health risk:<br/>
                    <strong style={{ color: "#fca5a5" }}>Class 1 (BMI 30–34.9) — Moderate:</strong> Lifestyle changes and medical support recommended.<br/>
                    <strong style={{ color: "#fca5a5" }}>Class 2 (BMI 35–39.9) — Severe:</strong> Medical intervention is often recommended.<br/>
                    <strong style={{ color: "#fca5a5" }}>Class 3 (BMI 40+) — Very Severe:</strong> Also called morbid obesity. Medical treatment is strongly advised.
                  </div>
                </div>
              )}
              ...
              {results.waistRisk && (
                <div className="waist-box">
                  <div>
                    <div className="stat-lbl">Waist Circumference Risk</div>
                    <div className="waist-hint">{sex === "male" ? "Men: Low <94cm | Increased 94–102cm | High >102cm" : "Women: Low <80cm | Increased 80–88cm | High >88cm"}</div>
                  </div>
            ...
            <div className="card">
              ...
              <div className="pi-box">
                ...
                <div className="pi-explain">
                  <strong>What is the Ponderal Index?</strong><br/><br/>
                  The Ponderal Index is an alternative to BMI that divides your weight by height <em>cubed</em> (kg/m³) instead of squared (kg/m²). This makes it more accurate for people who are very tall or very short — where BMI can give misleading results.<br/><br/>
                  <strong>Why does the cube matter?</strong><br/>
                  BMI treats height as a flat two-dimensional measure. But human bodies are three-dimensional. The Ponderal Index corrects for this — a very tall person with normal body fat won't be incorrectly labeled as overweight the way BMI sometimes does.<br/><br/>
                  <strong>Is it better than BMI?</strong><br/>
                  For people at height extremes, yes — it can be more accurate. But BMI remains the global medical standard used by doctors and researchers worldwide. Use both numbers together for the most complete picture.
                </div>
              </div>
            </div>
              ...
              <div style={{ marginBottom: "16px" }}>
                {results.risks.map((r, i) => (
                  <div key={i} className="risk-item">
                    <div className="risk-dot" style={{ background: r.c }}/>
                    <div className="risk-txt">{r.t}</div>
                  </div>
                ))}
              </div>
            ...
            <div className="card">
              ...
              {results.steps.map((s, i) => (
                <div key={i} className="step-item">
                  ...
                  <div className="step-txt"><strong>{s.h}</strong> — {s.d}</div>
                </div>
              ))}
              <div className="tools-grid">
                <a href="/nutrients" className="tl"><div className="tl-name">💊 Optimal Nutrients Calculator</div><div className="tl-desc">Get personalized daily nutrient targets</div></a>
            ...
            <div className="seo-section">
              ...
              <div className="seo-card">
                ...
                <div className="faq-item">
                  <div className="faq-q">What is a healthy BMI for adults?</div>
                  <div className="faq-a">For most adults a BMI between <strong>18.5 and 24.9</strong> is considered a healthy weight. Below 18.5 is underweight, 25 to 29.9 is overweight, and 30 or above is obese. However these thresholds are adjusted for people of Asian descent — overweight begins at 23 and obese at 27.5 due to evidence that health risks appear at lower BMI levels in this population.</div>
                </div>
                <div className="faq-item">
                  <div className="faq-q">Is BMI accurate for athletes and muscular people?</div>
                  <div className="faq-a">No — BMI is one of its least accurate for athletes and highly muscular individuals. Because muscle weighs more than fat, a very muscular person can have a <strong>high BMI while having very low body fat</strong>. This is a well-known limitation of BMI and why it should always be used alongside other measures like waist circumference and body fat percentage.</div>
                </div>
                <div className="faq-item">
                  <div className="faq-q">How is BMI calculated?</div>
                  <div className="faq-a">BMI is calculated by dividing your weight in kilograms by your height in meters squared: <strong>BMI = kg ÷ m²</strong>. For imperial measurements the formula is: weight in pounds multiplied by 703, then divided by height in inches squared. This calculator handles all the math — just enter your details and click Calculate.</div>
                </div>
                <div className="faq-item">
                  <div className="faq-q">What is the Ponderal Index and how is it different from BMI?</div>
                  <div className="faq-a">BMI divides weight by height squared (kg/m²). The Ponderal Index divides weight by height <strong>cubed</strong> (kg/m³). This makes it more accurate for people who are very tall or very short — where BMI tends to overestimate fatness in taller people and underestimate it in shorter people. A healthy Ponderal Index falls between 11 and 14. Both numbers are shown in your results so you can compare them.</div>
                </div>
                <div className="faq-item">
                  <div className="faq-q">Why does ethnicity affect BMI interpretation?</div>
                  <div className="faq-a">Research shows that people of Asian descent develop obesity-related health risks — including Type 2 diabetes and cardiovascular disease — at <strong>lower BMI levels than people of European descent</strong>. International health guidance recommends lower thresholds for Asian populations — overweight at 23 and obese at 27.5 rather than the standard 25 and 30. This is why our calculator offers ethnicity-adjusted results.</div>
                </div>
                <div className="faq-item">
                  <div className="faq-q">What does waist circumference tell me that BMI doesn't?</div>
                  <div className="faq-a">Waist circumference measures <strong>abdominal visceral fat</strong> — the fat stored around your organs that is far more dangerous than fat stored under the skin elsewhere. Two people can have the same BMI but very different waist sizes and health risks. For men a waist over 102cm (40 inches) indicates high risk. For women the threshold is 88cm (35 inches). Adding your waist measurement gives you a much more complete picture of your health.</div>
                </div>
                <div className="faq-item">
                  <div className="faq-q">Can I use this BMI calculator if I have diabetes?</div>
                  <div className="faq-a">Yes — this calculator is safe to use with any health condition. However if you have Type 2 diabetes, your weight management goals and safe ranges may differ from standard BMI recommendations. <strong>Always consult your doctor or a registered dietitian</strong> before making changes to your diet or exercise routine based on your BMI result. BMI is a screening tool, not a medical diagnosis.</div>
                </div>
              </div>
              ...
              <div className="int-box">
                ...
                <div className="tools-grid">
                  <a href="/nutrients" className="tl"><div className="tl-name">💊 Optimal Nutrients Calculator</div><div className="tl-desc">40+ personalized daily nutrient targets</div></a>
                  ...
                  <a href="/pregnancy" className="tl"><div className="tl-name">🤰 Pregnancy Due Date Calculator</div><div className="tl-desc">Calculate your due date 3 different ways</div></a>
